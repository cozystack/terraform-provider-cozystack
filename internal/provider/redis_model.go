...
func (m *redisModel) expand(ctx context.Context) (*client.Application, diag.Diagnostics) {
	...
	spec := map[string]any{
		...
		"authEnabled":     m.AuthEnabled.ValueBool(),
		"resourcesPreset": m.ResourcesPreset.ValueString(),
...
func (m *redisModel) flatten(app *client.Application) diag.Diagnostics {
	...
	m.StorageClass = types.StringValue(specString(app.Spec, "storageClass"))
	m.External = types.BoolValue(specBool(app.Spec, attrExternal))
	m.Version = types.StringValue(specString(app.Spec, attrVersion))
	m.AuthEnabled = types.BoolValue(specBool(app.Spec, "authEnabled"))
	m.ResourcesPreset = types.StringValue(specString(app.Spec, "resourcesPreset"))
