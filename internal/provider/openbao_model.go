...
// attributes mirror the json tags of the pinned openbao.ConfigSpec one-to-one.
...
type openbaoResourceModel struct {
	openbaoModel
...
func (m *openbaoResourceModel) waitConfig() (types.Bool, types.String) {
	return m.WaitForReady, m.WaitTimeout
}

// identity returns the openbao instance's namespace and name.
func (m *openbaoModel) identity() (string, string) {
...
func (m *openbaoModel) expand(ctx context.Context) (*client.Application, diag.Diagnostics) {
...
// flatten populates the Terraform model from the server view of an openbao.
func (m *openbaoModel) flatten(app *client.Application) diag.Diagnostics {
	var diags diag.Diagnostics
	...
	m.UI = types.BoolValue(specBool(app.Spec, "ui"))
