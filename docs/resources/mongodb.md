...
page_title: "cozystack_mongodb Resource - cozystack"
subcategory: ""
description: |-
  A Cozystack managed MongoDB instance, deployed inside a tenant namespace. The deprecated backup block, the bootstrap restore block, and detailed shardingConfig are not managed.
---

# cozystack_mongodb (Resource)
...
- `name` (String) MongoDB instance name (`metadata.name`). Immutable.
...
- `replicas` (Number) Number of MongoDB replicas.
...
- `sharding` (Boolean) Enable sharding (detailed shardingConfig uses server defaults).
...
- `version` (String) MongoDB major version (`v8`, `v7`, `v6`).
...
```shell
# MongoDB instances are imported as "<namespace>/<name>".
terraform import cozystack_mongodb.app tenant-root/app
```
